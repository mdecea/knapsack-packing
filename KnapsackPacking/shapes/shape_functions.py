...
def get_bounding_rectangle_center(shape):
    ...
    return (shape.bounds[0] + shape.bounds[2]) / 2, (
...
def get_shape_exterior_points(shape, is_for_visualization=False):
    ...
    if type(shape) == LinearRing:
        return [coord[0] for coord in shape.coords], [
            coord[1] for coord in shape.coords
        ]
    ...
    if type(shape) == Circle:
        if is_for_visualization:
            return shape.center.buffer(
                shape.radius, VISUALIZATION_RESOLUTION
            ).exterior.xy
...
def create_random_triangle_in_rectangle_corner(min_x, min_y, max_x, max_y):
    ...
    triangle_points = [
        first_vertex,
        (first_vertex[0], random.uniform(min_y, max_y)),
        (random.uniform(min_x, max_y), first_vertex[1]),
    ]
...
def create_random_quadrilateral_in_rectangle_corners(min_x, min_y, max_x, max_y):
    ...
    if has_horizontal_segment:
        quadrilateral_points = [
            (min_x, random.uniform(min_y, max_y)),
            (max_x, random.uniform(min_y, max_y)),
        ]
        ...
        quadrilateral_points = [
            (random.uniform(min_x, max_x), min_y),
            (random.uniform(min_x, max_x), max_y),
        ]
        if bool(random.getrandbits(1)):
