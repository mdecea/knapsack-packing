...
class PlacedShape(object):
    ...
    def __init__(self, shape, position=(0.0, 0.0), rotation=0.0, move_and_rotate=True):
        """Constructor"""
        ...
        if move_and_rotate:
            ...
            self.move(
                (
                    position[0] - bounding_rectangle_center[0],
                    position[1] - bounding_rectangle_center[1],
                ),
                False,
            )
    ...
    def __deepcopy__(self, memo=None):
        ...
        return PlacedShape(
            self.shape, copy.deepcopy(self.position), self.rotation, False
        )
    ...
    def move(self, displacement, update_reference_position=True):
        ...
        if displacement != (0.0, 0.0) or (
            type(self.shape) == Ellipse
            and get_bounding_rectangle_center(shape_to_move) != self.shape.center
        ):
            shape_to_move = affinity.translate(
                shape_to_move, displacement[0], displacement[1]
            )
        ...
        if type(self.shape) == Circle:
            center_displacement = (
                self.shape.center.x - self.shape.polygon.centroid.x,
                self.shape.center.y - self.shape.polygon.centroid.y,
            )
            if center_displacement != (0, 0):
                self.shape.polygon = affinity.translate(
                    self.shape.polygon, center_displacement[0], center_displacement[1]
                )
    ...
    def move_to(self, new_position):
        ...
        self.move(
            (new_position[0] - self.position[0], new_position[1] - self.position[1])
        )
    ...
    def rotate(self, angle, update_reference_rotation=True, origin=None):
        ...
        if (
            not np.isnan(angle)
            and angle != 0
            and (type(self.shape) != Circle or origin is not None)
        ):
            # for the ellipse, apply the action to the approximate polygon
            if type(self.shape) == Ellipse:
...
class Solution(object):
    ...
    def __init__(self, problem, placed_items=None, weight=0.0, value=0.0):
        """Constructor"""
    ...
    def __deepcopy__(self, memo=None):
        ...
        return Solution(
            self.problem,
            {
                index: copy.deepcopy(placed_item)
                for index, placed_item in self.placed_items.items()
            },
            self.weight,
            self.value,
        )
    ...
    def get_area(self):
        ...
        return sum(
            placed_shape.shape.area for _, placed_shape in self.placed_items.items()
        )
    ...
    def get_random_placed_item_index(self, indices_to_ignore=None):
        ...
        if not indices_to_ignore:
            ...
            valid_placed_item_indices = [
                item_index
                for item_index in self.placed_items.keys()
    ...
    def add_item(self, item_index, position, rotation=np.nan):
        ...
        if (
            0 <= item_index < len(self.problem.items)
            and item_index not in self.placed_items
        ):
    ...
    def _move_item_to(
        self, item_index, new_position, has_checked_item_in_container=False
    ):
            ...
            if has_checked_item_in_container or item_index in self.placed_items:
                ...
                line = LineString(
                    [
                        ...
                        (
                            placed_item.position[0] + direction[0] * max_dist_to_check,
                ...
                if intersection_points:
                    ...
                    intersection_point, min_dist = min(
                        [
                            (
                                ...
                                np.linalg.norm(
                                    (
                                        placed_item.position[0] - p[0],
                                        placed_item.position[1] - p[1],
                                    )
                                ),
                            )
                    ...
                    if min_dist >= min_dist_to_check:
                        ...
                        for i in range(point_num - 1):
                            points_to_check.append(
                                (
                                    ...
                                    + direction[0] * i * iter_dist,
                                    placed_item.position[1]
                                    + direction[1] * i * iter_dist,
                                )
                            )
                        ...
                        while nearest_point_index < furthest_point_index:
                            middle_point_index = (
                                nearest_point_index + furthest_point_index
                            ) // 2
                            if self.move_item_to(
                                item_index, points_to_check[middle_point_index]
                            ):
    ...
    def swap_placements(
        self, item_index0, item_index1, swap_position=True, swap_rotation=True
    ):
        ...
        if swap_position or swap_rotation:
            ...
            if (
                item_index0 != item_index1
                and item_index0 in self.placed_items
                and item_index1 in self.placed_items
            ):
                ...
                if self.is_valid_placement(item_index0) and self.is_valid_placement(
                    item_index1
                ):
                    return True
            ...
            if type(item.shape) == MultiPolygon:
                ...
                for other_index, placed_shape in self.placed_items.items():
                    if other_index != item_index:
                        for hole in holes:
                            if does_shape_contain_other(
                                hole, self.placed_items[other_index].shape
                            ):
    ...
    def visualize(
        ...
        title_override=None,
        show_title=True,
        show_container_value_and_weight=True,
        show_outside_value_and_weight=True,
        show_outside_items=True,
        color_items_by_profit_ratio=True,
        show_item_value_and_weight=True,
        show_value_and_weight_for_container_items=False,
        show_reference_positions=False,
        show_bounding_boxes=False,
        show_value_weight_ratio_bar=True,
        force_show_color_bar_min_max=False,
        show_plot=True,
        save_path=None,
    ):
        ...
        if show_title:
            fig.suptitle(
                title_override
                if title_override
                else "2D Irregular Shape Packing + 0/1 Knapsack Problem"
            )

        outside_item_bounds = dict()
        total_outside_item_width = 0.0
        ...
        for item_index, item in self.problem.items.items():
            ...
            if item_index in self.placed_items:
                if color_items_by_profit_ratio:
                    fill_color = worst_profit_color + tuple(
                        best_profit_color[i] - worst_profit_color[i]
                        for i in range(len(best_profit_color))
                    ) * color_interp(item_profit_ratios[item_index])
                else:
                ...
                self.show_item(
                    ...
                    fill_color,
                    container_color,
                    show_item_value_and_weight
                    and show_value_and_weight_for_container_items,
                    font,
                    show_bounding_boxes,
                    show_reference_positions,
                )
            ...
            # determine the boundary rectangle of the outside-of-container items
                ...
                total_outside_item_width += abs(
                    outside_item_bounds[item_index][2]
                    - outside_item_bounds[item_index][0]
                )
        ...
        if show_outside_items and ax2:
            ...
            row_num = max(
                1,
                int(
                    np.log10(len(self.problem.items))
            ...
            for item_index, bounds in outside_item_bounds.items():
                ...
                position_offset = (
                    width + shape_width * 0.5 - shape_center[0],
                    row_height + shape_height * 0.5 - shape_center[1],
                )
                self.show_item(
                    item_index,
                    ax2,
    ...
    def show_item(
        ...
        fill_color,
        container_color,
        show_item_value_and_weight=False,
        font=None,
        show_bounding_box=False,
        show_reference_position=False,
        position_offset=(0, 0),
    ):
        ...
        if show_item_value_and_weight and font:
            ...
            item_font["size"] = 9
            ax.text(
                centroid.x + position_offset[0],
                centroid.y + position_offset[1],
                value_weight_string,
        ...
        if show_bounding_box:
            ...
            ax.plot(
                bounds_center[0] + position_offset[0],
                bounds_center[1] + position_offset[1],
                "r.",
            )
