...
from KnapsackPacking.shapes.shape_functions import (
    ...
    do_shapes_intersect,
    create_random_triangle_in_rectangle_corner,
...
def get_fittest_solutions(solutions, size, break_ties=True):
    ...
    if len(solutions_by_fitness) == size or get_fitness(
        solutions_by_fitness[size - 1]
    ) != get_fitness(solutions_by_fitness[size]):
        return solutions_by_fitness[:size]
...
def generate_initial_solution(
    ...
    return greedy.solve_problem(
        problem,
        greedy_score_function=greedy.get_constant_score,
        repetition_num=1,
        max_iter_num=INITIAL_SOLUTION_GENERATION_MAX_ITER_NUM,
        max_iter_num_without_changes=INITIAL_SOLUTION_GENERATION_CONVERGE_ITER_NUM,
...
def generate_population(
    ...
    feasible_item_indices = [
        index
        for (index, item) in problem.items.items()
        if item.weight <= problem.container.max_weight
    ]
...
def get_crossover(
    ...
    parent1,
    max_attempt_num,
    shape_min_length_proportion,
    shape_max_length_proportion,
    shape_min_area_proportion,
    polygon_max_vertex_num,
    max_permutation_num,
    min_fitness_for_non_best,
):
    ...
    while i < max_attempt_num and not has_valid_shape:
        ...
        has_valid_shape = (
            shape is not None
            and shape.area >= shape_min_area
            and do_shapes_intersect(parent0.problem.container.shape, shape)
        )
    ...
    if has_valid_shape:
        ...
        for item_index in (
            parent0_separated_item_indices[1] + parent0_separated_item_indices[2]
        ):
        ...
        for item_index in (
            parent0_separated_item_indices[0] + parent0_separated_item_indices[2]
        ):
        ...
        for parent_index, intersected_item_indices in enumerate(
            [parent0_separated_item_indices[2], parent1_separated_item_indices[2]]
        ):
        ...
        if item_parent_pairs:
            ...
            permutations = list(
                itertools.islice(
                    itertools.permutations(item_parent_pairs), max_permutation_num * 10
                )
            )
            ...
            for permutation_index, permutation in enumerate(permutations):
                ...
                candidate_offspring = (
                    copy.deepcopy(offspring0)
                    if derive_offspring_0
                    else copy.deepcopy(offspring1)
                )
                for item_index, parent_index in permutation:
                    parent = parent0 if parent_index == 0 else parent1
                    candidate_offspring.add_item(
                        item_index,
                        parent.placed_items[item_index].position,
                        parent.placed_items[item_index].rotation,
                    )
                candidate_fitness = get_fitness(candidate_offspring)
                best_candidate_index = (
                    best_candidate0_index
                    if derive_offspring_0
                    else best_candidate1_index
                )
                if best_candidate_index == -1 or candidate_fitness > get_fitness(
            ...
            if high_fitness_candidate_indices:
                return [offspring0, offspring1] + [
                    candidates[index]
                    for index in high_fitness_candidate_indices
                    if index != best_candidate0_index and index != best_candidate1_index
                ]
...
def mutate_with_addition(solution, max_attempt_num, rotation="none"):
    ...
    feasible_item_indices = [
        index
        for index in solution.problem.items.keys()
        if solution.problem.items[index].weight <= remaining_weight
        and index not in solution.placed_items.keys()
    ]
...
def mutate_with_placement_modification(
    solution,
    max_attempt_num,
    small_position_change_proportion,
    small_rotation_change_proportion,
    move_until_intersection_point_num,
    move_until_intersection_min_dist_proportion,
    ...
    if solution.placed_items:
        ...
        move_until_intersection_min_dist = np.linalg.norm(
            (
                x_length * move_until_intersection_min_dist_proportion,
                y_length * move_until_intersection_min_dist_proportion,
            )
        )
        ...
        (
            position_change_index,
            small_position_change_index,
            move_until_intersect_index,
            position_swap_index,
        ...
        for _ in range(max_attempt_num):
            ...
            if action_index == position_change_index:
                ...
                has_modified = solution.move_item(
                    item_index,
                    (
                        random.uniform(-max_small_x_change, max_small_x_change),
                        random.uniform(-max_small_y_change, max_small_y_change),
                    ),
                )
            elif action_index == move_until_intersect_index:
                has_modified = solution.move_item_in_direction(
                    item_index,
                    (random.uniform(-1, 1), random.uniform(-1, 1)),
                    move_until_intersection_point_num,
                    move_until_intersection_min_dist,
                    move_until_intersection_max_dist,
                )
                ...
                has_modified = solution.move_and_rotate_item(
                    ...
                    random.uniform(-max_small_rotation, max_small_rotation),
                )
...
def get_mutation(
    ...
    min_iter_num,
    mutation_add_weight,
    mutation_remove_weight,
    mutation_modify_weight,
    mutation_add_max_attempt_num,
    mutation_modify_max_attempt_num,
    small_position_change_proportion,
    small_rotation_change_proportion,
    ...
    while iter_count < min_iter_num or removal_num_to_compensate > 0:
        ...
        if action_index == add_index:
            ...
            has_mutated = mutate_with_placement_modification(
                mutated_solution,
                mutation_modify_max_attempt_num,
                small_position_change_proportion,
                small_rotation_change_proportion,
        ...
        if (
            has_mutated
            and mutation_intermediate_selection_prob > 0
            and get_fitness(mutated_solution) > get_fitness(best_solution)
        ):
    ...
    if (
        mutation_intermediate_selection_prob > 0
        and mutated_solution != best_solution
        and best_solution != solution
        and random.uniform(0, 1) < mutation_intermediate_selection_prob
    ):
...
def generate_offspring(
    ...
    mutation_modify_rotate_until_intersection_angle_num,
    mutation_intermediate_selection_prob,
    crossover_ignore_mutation_probability,
    crossover_max_attempt_num,
    crossover_shape_min_length_proportion,
    crossover_shape_max_length_proportion,
    crossover_shape_min_area_proportion,
    crossover_polygon_max_vertex_num,
    ...
    for individual in individuals_to_mutate:
        ...
        if (
            can_use_crossover
            and random.uniform(0, 1) < crossover_ignore_mutation_probability
        ):
            offspring.append(individual)

        else:
            # create a mutated copy of the individual
            mutated_individual = get_mutation(
                individual,
                mutation_min_iter_num,
                mutation_add_weight,
                mutation_remove_weight,
            ...
            if (
                can_use_crossover
                and get_fittest_solution([individual, mutated_individual]) == individual
            ):
                offspring.append(individual)
...
def get_fitness_stats(population):
    ...
    return {
        "max": max_fitness,
        "min": min_fitness,
        "avg": fitness_sum / len(population),
        "mode": max(fitness_counts, key=fitness_counts.get),
    }
...
def solve_problem(
    ...
    initial_generation_item_specialization_iter_proportion=INITIAL_SOLUTION_GENERATION_FIRST_ITEM_SPECIALIZATION_ITER_PROPORTION,
    offspring_size=OFFSPRING_SIZE,
    elite_size=ELITE_SIZE,
    parent_selection_pool_size=PARENT_SELECTION_POOL_SIZE,
    population_update_pool_size=POPULATION_UPDATE_POOL_SIZE,
    max_generation_num=MAX_GENERATION_NUM,
    converge_generation_num=CONVERGE_GENERATION_NUM,
    mutation_min_iter_num=MUTATION_MIN_ITER_NUM,
    mutation_add_weight=MUTATION_ADD_WEIGHT,
    mutation_remove_weight=MUTATION_REMOVE_WEIGHT,
    mutation_modify_weight=MUTATION_MODIFY_WEIGHT,
    mutation_add_max_attempt_num=MUTATION_ADD_MAX_ATTEMPT_NUM,
    mutation_modify_max_attempt_num=MUTATION_MODIFY_MAX_ATTEMPT_NUM,
    small_position_change_proportion=MUTATION_MODIFY_SMALL_POSITION_CHANGE_PROPORTION,
    small_rotation_change_proportion=MUTATION_MODIFY_SMALL_ROTATION_CHANGE_PROPORTION,
    mutation_modify_move_until_intersection_point_num=MUTATION_MODIFY_MOVE_UNTIL_INTERSECTION_POINT_NUM,
    mutation_modify_move_until_intersection_min_dist_proportion=MUTATION_MODIFY_MOVE_UNTIL_INTERSECTION_MIN_DIST_PROPORTION,
    mutation_modify_rotate_until_intersection_angle_num=MUTATION_MODIFY_ROTATE_UNTIL_INTERSECTION_ANGLE_NUM,
    mutation_intermediate_selection_prob=MUTATION_INTERMEDIATE_SELECTION_PROB,
    can_use_crossover=CAN_USE_CROSSOVER,
    crossover_ignore_mutation_probability=CROSSOVER_IGNORE_MUTATION_PROBABILITY,
    crossover_max_attempt_num=CROSSOVER_MAX_ATTEMPT_NUM,
    crossover_shape_min_length_proportion=CROSSOVER_SHAPE_MIN_LENGTH_PROPORTION,
    crossover_shape_max_length_proportion=CROSSOVER_SHAPE_MAX_LENGTH_PROPORTION,
    crossover_shape_min_area_proportion=CROSSOVER_SHAPE_MIN_AREA_PROPORTION,
    crossover_polygon_max_vertex_num=CROSSOVER_POLYGON_MAX_VERTEX_NUM,
    crossover_max_permutation_num=CROSSOVER_MAX_PERMUTATION_NUM,
    ...
    start_time = (
        ...
        parent_selection_time
    ) = (
        min_fitness_for_crossover_non_best_time
    ) = (
        crossover_time
    ) = (
        mutation_time
        ...
        elite_finding_time
    ) = population_update_time = early_stop_time = fitness_per_generation_time = 0
    ...
    minimal_value_difference = (
        min([item.value for item in problem.items.values()]) * 0.00001
    )
    ...
    for i in range(max_generation_num):
        ...
        if can_use_crossover and crossover_min_fitness_for_non_best_proportion > 0:
            ...
            min_fitness_index = (
                0
                if need_to_improve_best
                else math.ceil(
                    (1.0 - crossover_min_fitness_for_non_best_proportion)
                    * len(population)
                )
            )
        ...
        # generate offspring
        offspring_result = generate_offspring(
            parents,
            mutation_min_iter_num,
            mutation_add_weight,
            mutation_remove_weight,
        ...
        population = elite + get_surviving_population(
            ...
            population_size - len(elite),
            population_update_pool_size,
        )
    ...
    if calculate_times:
        approx_total_time = (
            ...
            + mutation_time
            + elite_finding_time
            + population_update_time
            + early_stop_time
            + fitness_per_generation_time
        )
        time_dict = {
            "Generation of the initial population": (
                initial_population_time,
                initial_population_time / approx_total_time,
            ),
            "Parent selection": (
                parent_selection_time,
                parent_selection_time / approx_total_time,
            ),
            "Min-fitness-for-crossover-non-best calculation": (
                min_fitness_for_crossover_non_best_time,
                min_fitness_for_crossover_non_best_time / approx_total_time,
            ),
            "Crossover": (crossover_time, crossover_time / approx_total_time),
            "Mutation": (mutation_time, mutation_time / approx_total_time),
            "Elite finding": (
                elite_finding_time,
                elite_finding_time / approx_total_time,
            ),
            "Population update": (
                population_update_time,
                population_update_time / approx_total_time,
            ),
            "Early stopping check": (
                early_stop_time,
                early_stop_time / approx_total_time,
            ),
            "Population fitness per generation gathering": (
                fitness_per_generation_time,
                fitness_per_generation_time / approx_total_time,
            ),
        }
        if return_population_fitness_per_generation:
